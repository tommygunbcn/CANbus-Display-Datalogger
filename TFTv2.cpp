...
void TFT::setCol(INT16U StartCol,INT16U EndCol)
{
    ...
    sendCMD(0x2A);                                                      /* Column Command address    vdv   */
	sendData(StartCol);
    sendData(EndCol);
...
{
	sendCMD(0x2B);                                                      /* Column Command address     vdv  */
	sendData(StartPage);
	sendData(EndPage);
}
...
INT16U TFT::getDisplayXSize()
{
	return(MAX_X);
}

INT16U int TFT::getDisplayYSize()
{
	return(MAX_Y);
}
uint8_t TFT::getFontXsize()
{
	return FONT_X;
}

uint8_t TFT::getFontYsize()
{
	return FONT_Y;
}
...
void TFT::drawChar( INT8U ascii, INT16U poX, INT16U poY,INT16U size, INT16U fgcolor, INT16U back_fgcolor)
{
    ...
    for (int i =0; i<FONT_X; i++ ) {
		...
		//INT8U temp = pgm_read_byte(&SmallFont1[ascii - 0x20][i]);
        for(INT8U f=0;f<8;f++)
        {
			fillRectangle(poX + i * size, poY + f * size, size, size, back_fgcolor);
            if((temp>>f)&0x01)
            {
...
void TFT::drawString(const char *string, INT16U stl, INT16U poX, INT16U poY, INT16U size, INT16U fgcolor, INT16U back_fgcolor)
{
	char buffer[20]="";
	...
	drawString(buffer, length, poX, poY, size, fgcolor, back_fgcolor);
	
	
}

void TFT::drawString(char *string, INT16U stl,INT16U poX, INT16U poY, INT16U size,INT16U fgcolor, INT16U back_fgcolor)
{
	...
	if (poX == RIGHT)
		poX = (MAX_X + 1) - (stl*(FONT_SPACE*size + 2));
	else if (poX == CENTER)
		poX = ((MAX_X + 1) - (stl*(FONT_SPACE*size + 2))) / 2;

	//erase previous values
	//fillRectangle(poX , poY , poX+ (stl * (FONT_SPACE*size + 2)), poY+ FONT_Y*size, BLACK);

   // while(*string)
	while(stl)
    {
        drawChar(*string, poX, poY, size, fgcolor, back_fgcolor);
        *string++;

        if(poX < MAX_X)
        {
            poX += FONT_SPACE*size+2;                                     /* Move cursor right            */
        }
		stl--;
		//length--;
    }
...
void TFT::drawString(String string, INT16U poX, INT16U poY, INT16U size, INT16U fgcolor, INT16U back_fgcolor)
{
	char buf[35];
	INT16U length;
	...
	string.toCharArray(buf, length+1);
	drawString(buf, length, poX, poY, size, fgcolor, back_fgcolor);
...
INT8U TFT::drawNumber(long long_num,INT16U poX, INT16U poY,INT16U size,INT16U fgcolor, INT16U back_fgcolor)
{
    ...
    {
        f=1;
        drawChar('-',poX, poY, size, fgcolor, back_fgcolor);
        long_num = -long_num;
        if(poX < MAX_X)
        {
    ...
    {
        f=1;
        drawChar('0',poX, poY, size, fgcolor, back_fgcolor);
        return f;
        if(poX < MAX_X)
        {
            poX += FONT_SPACE*size+2;                                     /* Move cursor right            */
    ...
    for(; i > 0; i--)
    {
        drawChar('0'+ char_buffer[i - 1],poX, poY, size, fgcolor, back_fgcolor);
        if(poX < MAX_X)
        {
            poX+=FONT_SPACE*size+2;                                       /* Move cursor right            */
        }
...
INT8U TFT::drawFloat(float floatNumber,INT8U decimal,INT16U poX, INT16U poY,INT16U size,INT16U fgcolor, INT16U back_fgcolor)
{
    INT16U temp=0;
    float decy=0.0;
    float rounding = 0.5;
    ...
    INT8U howlong=drawNumber(temp,poX, poY, size, fgcolor, back_fgcolor);
    f += howlong;
    ...
    for(INT8U i=0;i<decimal;i++)                                      
    {
        ...
        drawNumber(temp,poX, poY, size, fgcolor, back_fgcolor);
        floatNumber = -floatNumber;
        if(poX < MAX_X)
        {
            poX+=FONT_SPACE*size;                                       /* Move cursor right            */
...
INT8U TFT::drawFloat(float floatNumber,INT16U poX, INT16U poY,INT16U size,INT16U fgcolor, INT16U back_fgcolor)
{
    INT8U decimal=2;
    INT16U temp=0;
    float decy=0.0;
    ...
    {
        drawChar('-',poX, poY, size, fgcolor, back_fgcolor);                          /* add a '-'                    */
        floatNumber = -floatNumber;
        if(poX < MAX_X)
        {
            poX+=FONT_SPACE*size;                                       /* Move cursor right            */
